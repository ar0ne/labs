...
Sha256.hash = function(msg) {
    ...
    // конвертирует строку msg в 512-бит по 16-бит блоки массивами int'ов
    var l = msg.length/4 + 2; // длина (в 32-bit) сообщения + ‘1’ + добавленная длина
    var N = Math.ceil(l/16); 
    var M = new Array(N);

    for (var i=0; i < N; i++) {
        M[i] = new Array(16);
        for (var j=0; j < 16; j++) { // big-endian encoding
            M[i][j] = (msg.charCodeAt(i*64 + j*4)     << 24) | (msg.charCodeAt(i*64 + j*4 + 1) << 16) | 
                      (msg.charCodeAt(i*64 + j*4 + 2) <<  8) | (msg.charCodeAt(i*64 + j*4 + 3));
        } 
    }
    // добавляем длину в битах в конец
    ...
    for (var i=0; i < N; i++) {
        ...
        // подготавливаем сообщение "W"
        for (var t=0;  t<16; t++) W[t] = M[i][t];
        for (var t=16; t<64; t++) W[t] = (Sha256.sigma1(W[t-2]) + W[t-7] + Sha256.sigma0(W[t-15]) + W[t-16]) & 0xffffffff;

        // инициализируем переменные a, b, c, d, e, f, g, h с предыдущего значения хэша
        ...
        // вычисляем новые промежуточные значения хэша
